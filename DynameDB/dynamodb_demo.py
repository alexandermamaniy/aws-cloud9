...
class DynamoDBDemo:
    ...
    def update_item(self,table_name,key,update_expression,update_val):
        try:
            dynamodb_resource=boto3.resource('dynamodb')
            table=dynamodb_resource.Table(table_name)
            table.update_item(
                Key=key,
                UpdateExpression=update_expression, #SET album = :new_album
                ExpressionAttributeValues=update_val #:new_album: best of pink floyd
            )        
...
def main():
    ...
    update_expression='SET album = :new_album'
    expression_attribute={
        ':new_album':'Best Holis'
    }
    
    d.update_item(table_name, key, update_expression, expression_attribute)
